...
void play_music(uint32_t IRval) {
  int music = code(IRval);
  if (music == -1) return;

  switch (music + 1) {
    case 1: RULE(); break;
    case 2: BAD_APPLE(); break;
    case 3: ORDER(); break;
    case 4: NEVER_BE_ALONE(); break;
    case 5: BASICS_IN_BEHAVIOR(); break;
    case 6: EPOCH(); break;
    case 7: CARELESS_WHISPER(); break;
  }
}

int code(uint32_t IRval) {
  for (int i = 0; i <= 8; i++) {
    if (IRval == code_music[i]) {
      return i;
    }
  }
  return -1;
